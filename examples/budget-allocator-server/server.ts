...
interface BudgetCategory {
  ...
  trendPerMonth: number; // For generating realistic historical drift
}
...
server.registerTool(
  ...
  async (): Promise<CallToolResult> => {
    ...
    return {
      content: [
        { type: "text", text: formatBudgetSummary(response as BudgetDataResponse) },
      ],
...
const httpServer = app.listen(PORT, () => {
  console.log(`Budget Allocator Server listening on http://localhost:${PORT}/mcp`);
});
