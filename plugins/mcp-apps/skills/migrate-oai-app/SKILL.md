...
- [ ] Look at the build config (config files, env vars, etc.) and find the specific configuration mechanism your codebase uses for dev vs prod URLs. Compare that mechanism with the mechanism the CSP code uses for including dev origins only in development. Ensure that changes to the build config will be reflected in the CSP.
