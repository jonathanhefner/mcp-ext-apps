...
- [ ] Look at the build config (config files, env vars, etc.) and find every setting that affects dev vs prod URLs. Double-check that each setting affects dev vs prod CSP origins in the exact same way. You must justify out loud each hard-coded CSP origin.
